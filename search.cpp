...
Move counterMoves[MAX_THREADS][64][64];
...
{
    Move empty{-1, -1, NONE, NONE, NONE, FLAG_NONE};

    for (int from = 0; from < 64; from++)
        for (int to = 0; to < 64; to++)
            counterMoves[0][from][to] = empty;
}
...
void initCounterMovesThreaded(int numThreads)
{
    Move empty{-1, -1, NONE, NONE, NONE, FLAG_NONE};
    ...
    for (int threadId = 0; threadId < numThreads; threadId++)
        for (int from = 0; from < 64; from++)
            for (int to = 0; to < 64; to++)
                counterMoves[threadId][from][to] = empty;
}
...
// Mate score helpers
// ------------------------------------------------------------
static const int SEARCH_MATE = 1000000;
static const int SEARCH_MATE_THRESHOLD = 900000;

static int adjustMateScoreForTTStore(int score, int ply)
{
...
{
    // Captures AND promotions go through SEE.
    // Quiet moves fall through to history.
    if (move.captured != NONE || move.promotion != NONE)
    {
        ...
        if (seeValue >= 0)
        {
            ...
            // order by SEE, tie-broken by MVV-LVA.
            return 1000000
                 + seeValue * 10
        ...
        {
            // Losing capture: push way below all quiets.
            // History is always >= 0 in this engine, so a
            // negative SEE score sorts after every quiet.
            return seeValue;
        }
    }
...
)
{
    std::sort(
        moves.begin(),
        moves.end(),
        [&board, color, threadId](const Move& a, const Move& b)
        {
            return scoreMoveForOrdering(board, a, color, threadId)
                 > scoreMoveForOrdering(board, b, color, threadId);
        }
    );
}
...
)
{
    ...
    else
    {
        ...
        moves = generator.generateLegalCaptures(board);
    }

    orderMoves(board, moves, board.isWhiteToMove() ? 0 : 1, threadId);

    const int DELTA_MARGIN = 200;

    for (const Move& move : moves)
    {
        if (!inCheck && move.captured != NONE)
        {
            int bestCase =
                standPat +
                getPieceValue(move.captured) +
        ...
        // SEE pruning (new)
        // ----------------------------------------------------
        // Cheap delta pruning already passed. Now do the
        // expensive, exact check: simulate the full exchange
        // on `to` and skip captures that lose material.
        //
        // Skipped when:
        //   - we're in check (every reply matters, no pruning)
        //   - the move is a promotion (a queening pawn's
        //     "loss" is often a winning tactic)
        //   - the move is not a capture (quiets aren't searched
        //     in non-check QS anyway)
        //
        // Threshold is 0: any capture whose exchange value is
        // strictly negative gets skipped. Stand-pat already
        // covered the option of not capturing, so this can't
        // lose us a real gain.
        if (!inCheck &&
            move.promotion == NONE &&
            move.captured != NONE &&
...
int negamax(
    ...
    bool allowNullMove,
    int threadId = 0,
    Move lastMove = {-1, -1, NONE, NONE, NONE, FLAG_NONE}
)
{
    // --------------------------------------------------------
    // Time check
    ...
    // --------------------------------------------------------
    // Transposition Table (probe early)
    // --------------------------------------------------------
    // Probing here lets us reuse the stored static eval
    // (avoids a full evaluate() call) and lets us bail
    // out before doing razoring / null-move work on a
    // node we already understand.

    uint64_t hash = board.getZobristHash();

    int ttScore = 0;
    Move ttMove{-1, -1, NONE, NONE, NONE, FLAG_NONE};
    int ttStaticEval = INT_MIN;
    ...
    // Static evaluation
    // --------------------------------------------------------
    // Reuse the TT value if we have one, otherwise compute
    // it. ttStaticEval is INT_MIN only when there was no
    // matching TT entry at all.

    int staticEval =
        (ttStaticEval != INT_MIN)
            ? ttStaticEval
            : evaluate(board);    
...
//
    ...
    // --------------------------------------------------------
    // If our static eval is far above beta, we're confident
    // the opponent can't drag us back down - just return
    // the eval as a fail-high.
    //
    // Guards:
    //   - depth <= 6      beyond this, tactics dominate
    //   - not in check    every reply matters
    //   - not PvNode      we don't know pv here, so no
    //                     additional restriction
    //
    // Margin scales linearly with depth so we're more
    // conservative at deeper searches (where tactics are
    // more likely to swing the eval).
    //
    if (!inCheck && depth <= 6)
    {
    ...
    depth <= 2)
{
    ...
    if (allowNullMove &&
        depth >= 3 &&
        !inCheck &&
        staticEval >= beta)   // only bother if we're already "doing well"
    {
        ...
        if (nullScore >= beta)
        {
            ...
            // Verification search
            // ------------------------------------------------
            // A null-move cutoff can be wrong in zugzwang
            // positions (endgame, blockade, etc). At high
            // enough depth, do a shallow real search from
            // the SAME position (no null move allowed) and
            // only trust the cutoff if the verification also
            // fails high.
            //
            // The verification uses allowNullMove = false,
            // so it can't recursively trigger null-move
            // pruning and loop forever.
            //
            if (depth >= 12)
            {
                int verifyDepth = depth / 3;
                ...
                // Verification failed - fall through and
                // search this node normally.
            }
            else
            {
                return beta;
    ...
    {
        Move counter =
            counterMoves[threadId][lastMove.from][lastMove.to];
        ...
        if (counter.from != -1)
        {
            ...
            {
                if (
                    moves[j].from == counter.from &&
                    moves[j].to == counter.to &&
                    moves[j].promotion == counter.promotion
                )
                {
                    std::swap(moves[0], moves[j]);
                    break;
         ...
         moveIndex++)
    {
        ...
        // Colour-to-move index used by the history table.
        // Constant across the loop, but cheap enough to
        // compute here - the compiler will hoist it.
        int moveColor = board.isWhiteToMove() ? 0 : 1;

        int moveHistory =
            historyTable[threadId][moveColor][move.from][move.to];

        // A move's history is considered "hot" if it has
        // already caused beta cutoffs many times at this
        // color. Hot moves are protected from every
        // pruning heuristic below. Tunable: raise to
        // prune more, lower to protect more.
        const int HISTORY_PRUNE_GATE = 2000;

        bool isHistoryHot = (moveHistory >= HISTORY_PRUNE_GATE);
        ...
        // ----------------------------------------------------
        // At shallow depths, moves that appear late in the
        // ordering are statistically very unlikely to beat
        // alpha. Skip them entirely, without making them.
        //
        // Guards (all must hold to prune):
        //   - depth <= 4       below this the ordering is
        //                      unreliable enough that we
        //                      don't trust "late = bad"
        //   - not in check     every reply matters
        //   - isQuiet          captures/promos are tactical
        //   - !isKiller        proven-good quiet move
        //   - moveIndex >= threshold
        //
        // Threshold grows with depth so we prune more at
        // depth 4 than at depth 1.
        //
        if (!inCheck &&
            depth <= 4 &&
            isQuiet &&
        ...
        if (alpha >= beta)
        {
            ...
            )
            {
                ...
                // Counter-move update: this quiet move just
                // refuted the opponent's lastMove.
                if (lastMove.from != -1)
                {
                    counterMoves[threadId][lastMove.from][lastMove.to]
                        = move;
    ...
    {
        int ttScoreToStore =
            adjustMateScoreForTTStore(bestScore, ply);
...
)
{
    ...
    Move bestMoveOverall{-1, -1, NONE, NONE, NONE, FLAG_NONE};
    bool haveMove = false;
         ...
         depth++)
    {
        ...
        // ----------------------------------------------------
        //
        // Depth 1 uses the full window (nothing to aspire to
        // yet). Later depths start at previousScore ± delta,
        // and widen by doubling delta when the search fails
        // low or high.
        ...
        {
            alpha = std::max(previousScore - delta, INT_MIN + 1);
            beta  = std::min(previousScore + delta, INT_MAX);
        }

        // ----------------------------------------------------
        // Search this depth, re-searching on aspiration fail
        // ----------------------------------------------------

        while (true)
        {
            bestScore = INT_MIN;
            bestMoveThisDepth = rootMoves[0];
            iterationAborted = false;

            int searchAlpha = alpha;

            for (size_t moveIndex = 0;
                 moveIndex < rootMoves.size();
                 moveIndex++)
            {
                if (checkTime())
                {
                ...
                const Move& move = rootMoves[moveIndex];
                ...
                if (bestScore > searchAlpha)
                    searchAlpha = bestScore;
            ...
            // ------------------------------------------------
            // Aspiration fail low: widen and retry
            // ------------------------------------------------
            ...
            if (bestScore <= alpha)
            {
                ...
                alpha = std::max(bestScore - delta, INT_MIN + 1);
                beta  = std::min(bestScore + delta, INT_MAX);
            ...
            // ------------------------------------------------
            // Aspiration fail high: widen and retry
            // ------------------------------------------------
            ...
            // Search succeeded within window - done with this depth
            break;
        }
        ...
        if (threadId == 0)
        {
            ...
            // We already know the first PV move from this completed iteration.
            pv.push_back(bestMoveThisDepth);

            // Now follow the TT from the position AFTER the best root move.
            Board pvBoard = board;

            pvBoard.makeMove(
                bestMoveThisDepth.from,
                bestMoveThisDepth.to,
                bestMoveThisDepth.promotion
            );

            std::vector<Move> rest =
                extractPV(pvBoard, generator, depth - 1);
            ...
            if (bestScore > SEARCH_MATE_THRESHOLD)
            {
                std::cout
                    << " score mate "
                    << ((SEARCH_MATE - bestScore + 1) / 2);
            }
            else if (bestScore < -SEARCH_MATE_THRESHOLD)
            {
                std::cout
                    << " score mate -"
                    << ((SEARCH_MATE + bestScore + 1) / 2);
