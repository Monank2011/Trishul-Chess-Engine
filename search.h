...
// counterMoves[threadId][from][to] = the move that most
// recently refuted the opponent move (from, to) at this
// thread's search. Indexed by the OPPONENT's move, so at a
// node we look up by the move that brought us here.
extern Move counterMoves[MAX_THREADS][64][64];
